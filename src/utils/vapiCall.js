export async function startVapiCall({ patientPhone, patientData, messages }) {
  ...
  Appointment Status: ${patientData.doctorName
      ? `ALREADY BOOKED — Dr. ${patientData.doctorName} (${patientData.specialty}) on ${patientData.date} at ${patientData.time}`
      : 'Not yet booked'}
  
  FULL CHAT HISTORY:
  ${messages.map(m => `${m.role === 'assistant' ? 'Assistant' : 'Patient'}: ${m.text}`).join('\n')}
  ...
  `
    ...
    const response = await fetch('/api/make-call', {
      ...
      body: JSON.stringify({ patientPhone, patientData, chatContext })
    })
